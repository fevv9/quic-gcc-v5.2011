/* Linux-specific atomic operations for Hexagon EABI.
...
/* Note: Implement byte, short and int versions of atomic operations.
   There is no support for "long long" (64-bit) operations as yet.

	The implementation here is not the fastest, but is considerably
   simpler than creating a sync.md file. See, for example, powerc
   or alpha, which both have the same load-locked/store-conditional
   insns that Hexagon has.
*/
...
static void __kernel_dmb (void)
{
	/* barrier, isync or syncht ? */
