...
#ifndef CALL_FRAME_INSTRUCTION_LABEL
#define CALL_FRAME_INSTRUCTION_LABEL "LCFI"
#endif
...
dwarf2out_cfi_label (void)
{
  ...
  if (dwarf2out_do_cfi_asm ())
    ...
    {
      ASM_GENERATE_INTERNAL_LABEL (label, CALL_FRAME_INSTRUCTION_LABEL, dwarf2out_cfi_label_num++);
...
output_loc_list (dw_loc_list_ref list_head)
{
  ...
  for (curr = list_head; curr != NULL; curr = curr->dw_loc_next)
	...
	continue;
      ...
      /* ++ Labels LCFI and LFB point to begenning of the function (Sundeep - Dec 03, 2009) */
      char *lfb  = strstr(curr->begin, FUNC_BEGIN_LABEL);
      char *lcfi = strstr(curr->end, CALL_FRAME_INSTRUCTION_LABEL);
      /* Labels are named something like `.*LFB5' and `.*LCFI5'.  The `.*' is generated by 
         ASM_GENERATE_INTERNAL_LABEL. Trying to avoid hard coding of the prefix. */
      if (lfb && lcfi && ((lfb - curr->begin) == (lcfi - curr->end)))
        {
          int offset = lfb - curr->begin;
          /* Make sure the prefix and suffix are identical */
          if ((strncmp(curr->begin, curr->end, offset) == 0) && (strcmp(lfb+3, lcfi+4) == 0))
            {
              continue;
            }
        }
