...
// _LSY_ get_mode_bounds (desc->mode, desc->signed_p, desc->mode, &mmin, &mmax);
// _LSY_ nmax = INTVAL (mmax) - INTVAL (mmin); 
/* _LSY_  Use the same mode 
  cond_under = simplify_gen_relational (LT, SImode, iv->extend_mode,
					iv->base, mmin);
  cond_over = simplify_gen_relational (GT, SImode, iv->extend_mode,
				       iv->base, mmax);
*/ 					   
 cond_under = simplify_gen_relational (LT, SImode, mode,
                                        iv->base, mmin);
 cond_over = simplify_gen_relational (GT, SImode, mode,
                                       iv->base, mmax); 					
  switch (cond)
    {
      case LE:
			 ...
			 enum rtx_code cond, struct niter_desc *desc)
{
  ...
  /* Check that both ivs belong to a range of a single mode.  If one of the
     ...
     mode.  */
  /* Use of SImode here seems arbitrary - see where it is being set... 
     Disabling the following block does what I want... */  
